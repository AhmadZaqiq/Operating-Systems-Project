...
void FirstComeFirstServed(vector<stProcess>& vProcesses, vector<GanttSegment>& GanttChart)
{
	int CurrentTime = 0;
	...
	for (stProcess& P : vProcesses)
	{
		...
		GanttChart.push_back(segment);
	}
}

void ShortestJobFirstPreemptive(vector<stProcess>& vProcesses, vector<GanttSegment>& GanttChart)
{
	int CurrentTime = 0;
	...
	while (Complete != vProcesses.size())
	{
		...
		if (!GanttChart.empty() && GanttChart.back().ProcessName == vProcesses[Shortest].Name)
		{
			GanttChart.back().EndTime = CurrentTime + 1;
		}
		else
		{
			GanttChart.push_back({ vProcesses[Shortest].Name, CurrentTime, CurrentTime + 1 });
		}
...
void RoundRobin(vector<stProcess>& vProcesses, int Quantum, vector<GanttSegment>& GanttChart)
{
	int CurrentTime = 0;
	bool Done;
	...
	do
	{
		...
		for (stProcess& P : vProcesses)
		{
			if (P.RemainingTime > 0)
			{
				...
				{
					GanttChart.push_back({ P.Name, CurrentTime, CurrentTime + Quantum });
				...
				{
					GanttChart.push_back({ P.Name, CurrentTime, CurrentTime + P.RemainingTime });
...
void PrintGanttChart(vector<GanttSegment>& GanttChart)
{
	cout << "\nGantt Chart:\n";
	...
	{
		const GanttSegment& segment = GanttChart[i];
		if (i == 0)
		{
...
void DisplayResult(vector<stProcess>& vProcess, string AlgorithmName, vector<GanttSegment>& GanttChart)
{
...
void ResetValues(vector<stProcess>& vProcesses, vector<GanttSegment>& GanttChart)
{
	for (stProcess& P : vProcesses)
	{
		P.FinishTime = 0;
...
void StartProgram(vector<stProcess> vProcesses, int Quantum, vector<GanttSegment> GanttChart)
{
	int choice = 0;
	...
	do
	{
		...
		case 1:
			...
			system("cls");
			FirstComeFirstServed(vProcesses, GanttChart);
			DisplayResult(vProcesses, "First Come First Served", GanttChart);
			ResetValues(vProcesses, GanttChart);
		...
		case 2:
			...
			system("cls");
			ShortestJobFirstPreemptive(vProcesses, GanttChart);
			DisplayResult(vProcesses, "Shortest Job First 'Preemptive'", GanttChart);
			ResetValues(vProcesses, GanttChart);
			cout << "\n\nPress any key to return to the Main Menu ...";
			system("pause>0");
		...
		case 3:
			...
			system("cls");
			RoundRobin(vProcesses, Quantum, GanttChart);
			DisplayResult(vProcesses, "Round Robin", GanttChart);
			cout << "\n\nPress any key to return to the Main Menu ...";
			ResetValues(vProcesses, GanttChart);
			system("pause>0");
...
int main()
{
	...
	StartProgram(vProcesses, Quantum, GanttChart);
